...
- The API returns **200 OK**, with the response body in JSON format containing the requested data.

For unsuccessful requests:

- The API returns a **non-200 HTTP status code**.
- The response body is always JSON and includes:
  - `code` (string): a stable machine-readable error code  
  - `message` (string): a human-readable explanation of the error  

### Common error codes

| HTTP status | Code example                       | Meaning                                                                 |
|-------------|------------------------------------|-------------------------------------------------------------------------|
| 400         | `400_BAD_REQUEST_BODY`             | Instance value (\"x\") not found in enum                                |
| 400         | `400_INVALID_API_KEY`              | Missing, malformed, expired, or not recognized                          |
| 401         | `401_MISSING_AUTHENTICATION_TOKEN` | Often is wrong endpoint path                                            |
| 403         | `403_SUBSCRIPTION_INACTIVE`        | Subscription expired                                                    |
| 429         | `429_THROTTLED`                    | Too many requests wait some seconds and retry                           |
| 500         | `500_INTERNAL-ERROR`               | Unexpected server error                                                 |


```js
{
  "code": "400_BAD_REQUEST_BODY",
   "message": "Invalid request body [instance value (\"sanctins\") not found in enum (possible values: [\"pep\",\"sanctions\"])]"
}
