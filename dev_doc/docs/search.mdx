...
Filters allow refining search queries by targeting specific criteria.  
The following table lists all available filters for the `/check/sanctions_pep` endpoint.

| Filter | Type | Required | Description | Example |
|--------|------|-----------|--------------|----------|
| `group_type` | array[string] | optional | Determines the nature of the target — `"individual"` for persons, `"entity"` for organizations, `"vessel"` for ships, or `"aircraft"` for planes. Targets with unknown type are not filtered. | `["individual"]` |
| `nationality` | array[string] | optional | Must conform to ISO 3166-1 alpha-2 codes. Filters out nationalities or jurisdictions not matching the specified codes. | `["US", "GB"]` |
| `source_type` | array[string] | optional | Specifies databases to search: `"pep"` (Politically Exposed Persons), `"sanctions"`, or both. Empty defaults to all. | `["sanctions"]` |
| `gender` | array[string] | optional | Filters by gender. `"male"` excludes females and vice versa. Unknown gender is not filtered. | `["male"]` |
| `birth_year` | array[string] | optional | Year(s) of birth in `YYYY` format. Unknown birth years are not filtered. | `["1984"]` |
| `exclude_deceased` | boolean | optional | When `true`, excludes deceased individuals from results. Defaults to `true`. | `true` |
| `exclude_sanction_source_id` | array[string] | optional | Excludes results from specific sanction sources. Cannot be used with `include_only_sanction_source_id`. | `["AUCL"]` |
| `include_only_sanction_source_id` | array[string] | optional | Restricts results only to the specified sanction sources. Cannot be used with `exclude_sanction_source_id`. | `["AUCL"]` |
...
- **`rca_name`**  
Type: String. If rca_hit is true, then this is the name of the person which is related or close associated to the `name`.  
Example: `"Michelle Obama"`

- **`rca_hit`**  
Type: Boolean. The match was triggered by a related close associated to the `name`.  


- **`id_hit`**  
Type: Boolean. The match was triggered by an ID/passport/etc related to the matched profile.
